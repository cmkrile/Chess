...
public class Piece {
    ...
    public boolean isMoveLegal(Board board, int endRow, int endCol) {
        switch (this.character) {
            ...
            case '\u2656':
            case '\u265c':
                Rook rook = new Rook(character, row, col, isBlack);
                return rook.isMoveLegal(board, endRow, endCol);
            case '\u265e':
            case '\u2658':
                Knight knight = new Knight(character, row, col, isBlack);
                return knight.isMoveLegal(board, endRow, endCol);
            case '\u265d':
            case '\u2657':
                Bishop bishop = new Bishop(character, row, col, isBlack);
                return bishop.isMoveLegal(board, endRow, endCol);
            case '\u265b':
            case '\u2655':
                Queen queen = new Queen(character, row, col, isBlack);
                return queen.isMoveLegal(board, endRow, endCol);
            case '\u265a':
            case '\u2654':
                King king = new King(character, row, col, isBlack);
    ...
    public void promotePawn(int row, boolean isBlack) {
        if (row == 0 || row == 7) {
            if(this.character == '♟' || this.character == '♙') {
                ...
                String promotedPiece = scan.nextLine();
